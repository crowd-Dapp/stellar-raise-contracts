...
1. On success, listen for the `("campaign", "initialized")` event to confirm
   the campaign is live and cache the emitted parameters locally.
1. On failure, read the returned error code and call `describe_init_error(code)`
   to display a user-facing message.
1. Use `is_init_error_retryable(code)` to decide whether to show a "try again"
...
3. **Creator authentication** — `creator.require_auth()` is called before any
   storage write. The Soroban host rejects the transaction if the creator's
   signature is absent or invalid.

4. **Goal floor** — `goal >= 1` prevents zero-goal campaigns that could be
   immediately drained by the creator.

5. **Minimum contribution floor** — `min_contribution >= 1` prevents
   zero-amount contributions that waste gas and pollute storage.

6. **Deadline offset** — `deadline >= now + 60s` ensures the campaign is live
   for at least one ledger close interval, preventing dead-on-arrival campaigns.

7. **Platform fee cap** — `fee_bps <= 10_000` ensures the platform can never
   be configured to take more than 100% of raised funds.

8. **Bonus goal ordering** — `bonus_goal > goal` prevents a bonus goal that is
   already met at launch, which would immediately emit a bonus event and confuse
   contributors.

9. **Atomic write ordering** — All validations complete before the first
   `env.storage().instance().set()` call. A failed validation leaves the
   contract in its pre-initialization state.

10. **Description length validation** — Bonus goal description length is
...
11. **Description length validation** — Bonus goal description length is
    validated to prevent unbounded state growth that could increase storage
    costs and impact contract performance.
